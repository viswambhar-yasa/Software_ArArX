...
html.Div([
    ...
    def blank_assignment(value):
        if value=='Automatic':
            ...
            blank_layout = html.Div([html.Div([html.Div([html.Label("Blank Assignment table", style=dict(fontWeight='bold')),
                                               dash_table.DataTable(
                id="all_blank_table",
    ...
    def blank_exp_assigment(row):
        if row is not None:
            blank_exp=all_blanks.iloc[np.squeeze(row)][0]
            blank_type=all_blanks.iloc[np.squeeze(row)][1]
            blank_data['blank_experiment_no'] = blank_exp
            blank_data['blank_experiment_type']= blank_type
    ...
    def save_intercepts(n,exp,A36,A37,A38,A39,A40):
        if n is not None and n>=1:
            update_blank_intercepts(exp,np.squeeze(A36),np.squeeze(A37),np.squeeze(A38),np.squeeze(A39),np.squeeze(A40))
        return 0,0
    return blank_app
